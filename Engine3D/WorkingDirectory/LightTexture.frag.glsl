...
void main()
{
	...
	for (int index = 0; index < lightsNumber; index++) // for all light sources
	{
	...
	gl_FragColor = vec4(totalLighting, 1.0)  * texture2D(texture, uv);
}
...
void main()
{
	...
	gl_FragColor = vec4(ambientLighting + diffuseReflection + specularReflection, 1.0) * texture2D(texture, uv);
}
*/